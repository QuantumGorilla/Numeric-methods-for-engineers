...
z0=input("digite los valores iniciales para z");
...
c=1;
printf('i - X - Y - Z - Eax - Eay - Eaz - Di\n');
while (Di>= tol)
   
   xi(i)=double(subs(XiMas1(1),[y,z],[yi(i-1),zi(i-1)]));
   yi(i)=double(subs(XiMas1(2),[x,z],[xi(i-1),zi(i-1)]));
   zi(i)=double(subs(XiMas1(3),[x,y],[xi(i-1),yi(i-1)]));
